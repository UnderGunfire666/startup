	...
	if event is InputEventMouseButton:
		...
		if edit_mode != EditMode.SELECT:
			if mouse_button.pressed:
				_grid_drag_start = _screen_to_grid(mouse_button.position)
				_grid_dragging = true
			else:
				if _grid_dragging:
					var end_cell := _screen_to_grid(mouse_button.position)
...
func _draw() -> void:
	...
	for x in range(0, int(grid_size.x) + 1, int(grid_screen_size)):
		draw_line(Vector2(x, 0), Vector2(x, grid_size.y), Color(0.2, 0.23, 0.28, 0.35), 1.0)
	for y in range(0, int(grid_size.y) + 1, int(grid_screen_size)):
		draw_line(Vector2(0, y), Vector2(grid_size.x, y), Color(0.2, 0.23, 0.28, 0.35), 1.0)
	for cell in document.road_cells:
