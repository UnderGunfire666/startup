...
func _ready() -> void:
	...
	_assert(panel.document.add_road_segment("delete_test_segment", "tool_node", "road_cc_1"), "test road can be attached to a node before deletion")
	panel.node_id_input.text = "tool_node"
	...
	var polygon_cells: Array[Vector2i] = [Vector2i(202, 202), Vector2i(203, 202), Vector2i(202, 203)]
	...
	if polygon != null:
		_assert(polygon.has_map_point(panel.document.grid_to_world_center(Vector2i(202, 202))) and not polygon.has_map_point(panel.document.grid_to_world_center(Vector2i(203, 203))), "polygon block hit testing follows selected cells instead of its bounding rectangle")
