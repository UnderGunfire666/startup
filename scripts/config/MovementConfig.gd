...
static func get_travel_hours(from_block: BlockData, to_block: BlockData, road_graph: RoadGraph = null) -> float:
	...
	var distance := INF
	if road_graph != null and not from_block.road_entry_node_id.is_empty() and not to_block.road_entry_node_id.is_empty():
		distance = road_graph.get_shortest_distance(from_block.road_entry_node_id, to_block.road_entry_node_id)
	if is_inf(distance):
		distance = from_block.center_position.distance_to(to_block.center_position)
	if distance <= 0.0:
