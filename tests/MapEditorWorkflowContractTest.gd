...
func _ready() -> void:
	...
	block.internal_road_cells = [Vector2i(8, 6)]
	...
	_assert(imported_storefront != null and is_equal_approx(imported_storefront.monthly_rent_wan, 1.2) and is_equal_approx(imported_storefront.area, 10.0) and is_equal_approx(imported_storefront.footprint_area, 12.25) and is_equal_approx(imported_storefront.awareness_radius, 35.0) and is_equal_approx(imported_storefront.awareness_exposure_modifier, 1.25), "storefront business and awareness data survive export and import")
	...
	var neighboring_storefront_cells: Array[Vector2i] = [Vector2i(8, 7)]
